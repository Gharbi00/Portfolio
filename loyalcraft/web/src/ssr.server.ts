...
export function app() {
  ...
  const distFolder = join(process.cwd(), 'dist/apps/loyalcraft/web/browser');
  ...
  createProxyRoute(server, '/.well-known/apple-app-site-association');
  createProxyRoute(server, '/.well-known/assetlinks.json');
  server.use(
    '/meta-catalog.xml',
    createProxyMiddleware({
      target: bck_url,
