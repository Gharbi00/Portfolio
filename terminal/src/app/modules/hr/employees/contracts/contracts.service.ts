...
import { SortColumn, SortDirection } from './contracts-sortable.directive';
...
export class CustomersService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 4. Date Filter
    if (date) {
      customers = customers.filter((country) => this.datePipe.transform(country.date, 'yyyy-MM-dd') == this.datePipe.transform(date, 'yyyy-MM-dd'));
    } else {
      customers = customers;
    }
