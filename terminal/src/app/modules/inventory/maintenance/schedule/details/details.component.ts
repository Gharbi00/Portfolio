...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.scheduleService.card$.pipe(takeUntil(this.unsubscribeAll)).subscribe((card: BoardCardForListType) => {
      this.card = card;
  ...
  ngOnInit(): void {
    this.translate.get('MENUITEMS.TS.INVENTORY').subscribe((inventory: string) => {
      this.translate.get('MENUITEMS.TS.TASK_DETAILS').subscribe((taskDetails: string) => {
        this.breadCrumbItems = [{ label: inventory }, { label: taskDetails, active: true }];
      });
    });
  }
  ...
  openEditModal(content: any) {
    ...
    this.boardForm?.valueChanges.pipe(takeUntil(this.unsubscribeAll)).subscribe((val: any): void => {
      this.isBoardButtonDisabled = isEqual(val, initValues);
    });
  }
