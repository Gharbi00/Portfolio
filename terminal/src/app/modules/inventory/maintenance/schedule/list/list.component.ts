...
import { UserType, CompanyType, ProjectType, BoardCardType, BoardListType, BoardCardPriorityEnum } from '@sifca-monorepo/terminal-generator';
...
@Component({
  selector: 'sifca-monorepo-list-view',
...
export class ListViewComponent implements OnInit, OnDestroy {
  ...
  boardList$: Observable<BoardListType[]> = this.scheduleService.boardList$;
  boardCards$: Observable<BoardCardType[]> = this.scheduleService.boardCards$;
  companies$: Observable<CompanyType[]> = this.companiesService.infiniteLeads$;
  loadingBoardCards$: Observable<boolean> = this.scheduleService.loadingBoardCards$;
  ...
  constructor(
    ...
    this.scheduleService.pagination$.pipe(takeUntil(this.unsubscribeAll)).subscribe((pagination: IPagination) => {
      this.pagination = {
        length: pagination?.length,
        page: this.scheduleService.pageIndex || 0,
        size: this.scheduleService.filterLimit,
        lastPage: pagination?.length - 1,
        startIndex: (this.scheduleService.pageIndex || 0) * this.scheduleService.filterLimit,
        endIndex: Math.min(((this.scheduleService.pageIndex || 0) + 1) * this.scheduleService.filterLimit - 1, pagination?.length - 1),
      };
    ...
    this.searchForm.valueChanges
      .pipe(
        ...
        switchMap((searchValues) => {
          this.changeDetectorRef.markForCheck();
          this.scheduleService.searchString = searchValues.searchString;
          return this.scheduleService.getBoardCardsByBoardWithFilterPaginated(this.boardId);
        }),
      )
      .subscribe(() => {
        this.changeDetectorRef.markForCheck();
  ...
  ngOnInit(): void {
    ...
    this.translate.get('MENUITEMS.TS.INVENTORY').subscribe((inventory: string) => {
      this.translate.get('MENUITEMS.TS.TASK_LIST').subscribe((taskList: string) => {
        this.breadCrumbItems = [{ label: inventory }, { label: taskList, active: true }];
      });
    });
    ...
    const initValues = this.filterForm?.value;
    this.filterForm?.valueChanges.pipe(takeUntil(this.unsubscribeAll)).subscribe((val: any): void => {
      this.isFilterButtonDisabled = isEqual(val, initValues);
    });
  ...
  openFilterModal(content: any) {
    ...
    this.scheduleService.getBoardListByBoard(this.boardId).subscribe();
    this.modalService.open(content, { centered: true });
  }
  ...
  save() {
    ...
    if (!this.selectedCard) {
      const input: any = {
        ...FormHelper.getNonEmptyValues(omit(this.boardForm.value, 'customer', 'tags', 'assignedTo')),
      ...
      const input: any = {
        ...
        ...(this.boardForm.get('customer').value === this.selectedCard?.customer?.id ? {} : { customer: this.boardForm.get('customer').value }),
        ...FormHelper.getNonEmptyAndChangedValues(
          omit(this.boardForm.value, 'tags', 'customer', 'assignedTo'),
  ...
  onPageChange(page: number) {
    ...
    this.scheduleService.pageIndex = page - 1;
    if (this.pageChanged) {
      this.scheduleService.getBoardCardsByBoardWithFilterPaginated(this.boardId).subscribe();
    }
  }
  ...
  ngOnDestroy() {
    ...
    this.scheduleService.pageIndex = 0;
  }
}
