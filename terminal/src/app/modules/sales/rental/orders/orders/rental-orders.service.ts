...
function matches(country: OrdersModel, term: string, pipe: PipeTransform) {
  return (
    country.orderId.toLowerCase().includes(term.toLowerCase()) ||
...
export class OrdersService {
  ...
  constructor(private pipe: DecimalPipe, private datePipe: DatePipe) {
    ...
    // Api Data
    // this.ApiService.getOrderData().subscribe((data) => {
  ...
  private _search(): Observable<SearchResult> {
    ...
    if (date) {
      countries = countries.filter(
        (country) => this.datePipe.transform(country.orderDate, 'yyyy-MM-dd') == this.datePipe.transform(date, 'yyyy-MM-dd'),
      );
    } else {
      countries = countries;
    }
